(function ($) {
   ...
   */
   // our tracker function
  function trk(ip, agent, lang, url) {
  	(new Image()).src = path+"/sapag/bug?ip="+ip+"&lang="+lang+"&agent="+agent+"&url="+url;
  }
    ...
    attach : function(context, settings) {
      $(automatic_pager_selector, context).once().each(function() {
        ...
        if (count==0) {
          ...
          trk(userIP, agent, lang, articleurl);
            ...
            setTimeout(function(){ // dynamically inserted elements
              $(".view-row-sa").each(function(){
                ...
                if (!$(this).is("[id]")) {
                    ...
                    // track this newly loaded article
                    trk(userIP, agent, lang, articleurl);
